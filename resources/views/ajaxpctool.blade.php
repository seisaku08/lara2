...
<h1 class="p-2">@yield('title')</h1>
  ...
  {{-- <form method="post" action="ajaxpctool">
    @csrf --}}
<div class="container darkgray box1000">
    <div class="row">
      <div class="column col-8">
        <div class="row">
          <div class="col text-center p-1">
            ...
            <input type="date" id="ajax_seminar_day" name="seminar_day" value="{{$input->seminar_day}}{{ old('seminar_day') }}">
          </div>
        </div>
        <div class="row">
          <div class="col text-center p-1">
          <label>予約開始日</label>
          <input type="date" id="ajax_from" name="from" value="{{$input->from}}{{ old('from') }}">
          </div>
          <div class="col text-center p-1">
            <label>予約終了日</label>
            <input type="date" id="ajax_to" name="to" value="{{$input->to}}{{ old('to') }}">
          </div>
        </div>
      </div>
      <div class="column text-center align-middle p-1">
        ...
        {{-- <div><button id="days_check">検索</button></div> --}}
      </div>
    </div>
</div>
...
<div id='list'>
  {{ Form::open(['route' => 'addCart', 'id' => 'pctool']) }}
    ...
    <table class="table table-striped table-sm" id="pctool">
      <tr class="midashi" id="all_show_result">
        <th>　</th>
        <th>ID</th>
        <th>機材番号</th>
        {{-- <th>状態</th> --}}
...
<script src="http://localhost/laravel2/public/vendor/jquery/jquery.min.js"></script>
<script src="{{ asset('js/pctool_ajax.js') }}"></script>
